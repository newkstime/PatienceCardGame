...
class CardSorter():
    ...
    def construct_piles_subseqs(self):
        ...
        for card in range(self.CARDS_IN_DECK):
            ...
            flag = False
            for p in range(len(self.__piles)):
                top_card = self.__piles[p].get_top_card()
                if card_from_deck.compare(top_card) < 0:
                    flag = True
                    if p > 0:
                        previous_card = self.__piles[p - 1].get_top_card()
                    else:
                        previous_card = None
                    self.add_card_to_subseq(card_from_deck, previous_card)
                    self.__piles[p].add_card(card_from_deck)
                    break
            if not flag:
                pile = Pile()
                self.__piles.append(pile)
                length = len(self.__piles)
                if length == 1:
                    previous_card = None
                else:
                    previous_card = self.__piles[-2].get_top_card()
                self.add_card_to_subseq(card_from_deck, previous_card)
        ...
        4. If there was a SubSequence containing the prev_card, 
           ...
           b. If the prev_card is not at the head,then we must
              ...
              iii. Traverse the copied SubSequence from the head,
                   ...
                   by calling the remocve_first method.  Be sure to
    ...
    def find_subseq(self, prev_card):
        ...
        if prev_card is None:
            ...
            for sub in self.__subseqs:
                if sub.contains_card(prev_card):
                    return sub
            return None
    ...
    def display_longest_subseq(self):
        ...
        for sub in self.__subseqs:
            if sub.get_size() == len(self.__piles):
                print(sub)
    ...
    def make_sorted_deck(self):
        ...
        A. Outer Loop: Loop 52 times, once for each Card in the Deck.
           ...
           4. Get the lowest card from the top of Piles and add it to 
              the Deck.              
        """
        for c in range(self.CARDS_IN_DECK):
            low_pile = 0
            low_card = Card(3, 13)
            for p in range(len(self.__piles)):
                if not self.__piles[p].is_empty():
                    first_card = self.__piles[p].get_top_card()
                    if first_card.compare(low_card) < 0:
                        low_card = first_card
                        low_pile = p
            self.__deck.enqueue(low_card)
            self.__piles[low_pile].remove_card()
